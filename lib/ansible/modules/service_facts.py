    ...
    def _list_from_unit_files(self, systemctl_path, services):
        ...
        if rc != 0:
            ...
            for line in [svc_line for svc_line in stdout.split('\n') if '.service' in svc_line]:
                ...
                if service_name not in services:
                    rc, stdout, stderr = self.module.run_command(
                        "%s show %s --property=ActiveState" % (systemctl_path, service_name), use_unsafe_shell=True)
                    state = 'unknown'
                    if not rc and stdout != '':
                        state = stdout.replace('ActiveState=', '').rstrip()
                    services[service_name] = {
                ...
                rc, stdout, stderr = self.module.run_command("%s is-enabled %s" % (systemctl_path, service_name),
                                                             use_unsafe_shell=True)
                self.module.debug("Command: %s is-enabled %s, RC: %s, Stdout: %s, Stderr: %s" % (
                systemctl_path, service_name, rc, stdout, stderr))
